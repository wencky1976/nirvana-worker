...
function buildProxyUrl(mobile = false) {
  ...
  return { username: user, password: pass, type: proxyType };
}

function getProxyInfo(proxyConfig) {
  return { host: "us.decodo.com", port: 10001, username: proxyConfig.username, password: proxyConfig.password };
}
...
// ── Google Page Helpers ─────────────────────────────────
async function handleCaptcha(page, context, proxyConfig, log) {
  ...
  try {
    ...
    if (page.url().includes("/sorry/")) {
      log("captcha_failed", "Still on sorry page");
      return false;
    }
...
async function setupBrowserSession(params, log) {
  ...
  log("proxy_configured", `${proxyConfig.type} — ${proxyConfig.username} → us.decodo.com:10001`);
