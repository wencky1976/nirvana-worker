...
async function humanScroll(page, totalDistance, log) {
  ...
  while (scrolled < totalDistance) {
    // Variable scroll chunks — humans scroll unevenly
    const chunk = rand(80, 250);
    const speed = rand(30, 120); // ms between scroll events
    await page.mouse.wheel(0, chunk);
    scrolled += chunk;
    await page.waitForTimeout(speed);
    
    // 20% chance to pause and "read" something
    if (Math.random() < 0.2) {
...
async function handleCaptcha(page, context, proxyConfig, log, maxAttempts = 3) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    try {
      // Navigate back to the sorry page to get a fresh data-s token
      // (page.reload causes ERR_ABORTED in GoLogin — use goto instead)
      if (attempt > 1) {
        ...
        try {
          ...
          // If goto also fails, try clicking browser back + forward
          log("captcha_refresh_fallback", `goto failed: ${navErr.message}, trying keyboard refresh`);
          try {
            await page.keyboard.press("F5");
            await page.waitForLoadState("domcontentloaded", { timeout: 15000 });
          } catch {
            log("captcha_refresh_failed", "Could not reload page — trying with stale data-s");
          }
        }
