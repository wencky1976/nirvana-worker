/**
 * PGS (Project Good Speed) — Driving directions to GBP
 * Google Maps directions FROM random grid point TO business → interact with profile
 */
...
async function run(job) {
  ...
  try {
    ...
    const address = params.address || params.business_address || "";

    if (!businessName) throw new Error("Business name is required for PGS journey");
    ...
    // Build directions URL: from grid point to business
    const destination = encodeURIComponent(businessName + (address ? ', ' + address : ''));
    const mapsUrl = `https://www.google.com/maps/dir/${gridPoint.lat},${gridPoint.lng}/${destination}`;
    log("directions_url_generated", mapsUrl.slice(0, 120));
    ...
    log("navigating_directions", mapsUrl.slice(0, 120));
    await page.goto(mapsUrl, { waitUntil: "domcontentloaded", timeout: 30000 }).catch(() => {});
    await sleep(rand(3000, 5000));
    
    const finalUrl = page.url();
    log("directions_loaded", finalUrl.slice(0, 120));
    ...
    // DIRECTIONS INTERACTION — browse the route first
    log("viewing_route", "Browsing driving directions");
    await sleep(rand(3000, 5000));
    
    // Scroll through directions/route overview
    for (let i = 0; i < rand(2, 4); i++) {
      await Promise.race([humanScroll(page, isMobile, personality), sleep(5000)]).catch(() => {});
      await sleep(rand(2000, 4000));
    }
    interactions.push("route_viewed");

    // Try to click on the destination business name to open GBP
    try {
      const destLink = await page.$('a[data-destination], a:has-text("' + businessName.split(' ')[0] + '"), [data-value="' + businessName + '"], .tactile-s-destination, h1:has-text("' + businessName.split(' ')[0] + '")').catch(() => null);
      if (destLink) {
        await humanMouseMove(page, destLink, isMobile).catch(() => {});
        await destLink.click().catch(() => {});
        log("destination_clicked", "Opening business profile");
        await sleep(rand(3000, 5000));
        interactions.push("destination_clicked");
      } else {
        log("destination_link_not_found", "Continuing with directions page");
      }
    } catch {}
